...
export default function StoryList({ theme }: { theme: string }) {
  ...
  return (
    <div>
      ...
      {/* Botão "See more" envolvido em motion.div */}
      {hasMoreStories && (
        <motion.div
        ...
        >
          <button
            ...
            className="rounded-xl px-6 py-3 bg-gradient-to-r bg-[#B3BAFF] hover:bg-[#b3bbff9d] transition-all text-lg font-semibold text-white"
          >
      ...
      {/* Popup com todas as histórias com container e cards maiores */}
      {showMorePopup && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          ...
          >
            {/* Motion para o cabeçalho "All {theme} stories" */}
            <motion.div
              initial={{ y: -20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            {/* Motion para a grade de histórias */}
            <motion.div
              initial={{ y: -20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
